...
\paragraph{Come trovare basi di C(A), di R(A) e di N(A)}\hspace{1cm}\\
A$\in$M$_{m\times n}(\mathbb{C})$, rk A$=k$\\
A$\underset{EG}{\longrightarrow}$U forma ridotta di Gauss per A
\begin{enumerate}
    \item \textbf{U ha k colonne dominanti} (rk A$=k$) \\
        Siano esse: $\vec{u}_{i_1},\vec{u}_{i_2}, \dots, \vec{u}_{i_k}$\\
        A$=[\vec{a}_1, \vec{a}_2, \dots\vec{a}_n]\hspace{1cm}$U$=[\vec{u}_1, \vec{u}_2, \dots\vec{u}_n]$\\\\
        Una base B di C(A) è l'insieme \textbf{delle colonne di A} corrispondenti alle colonne dominanti di U.
        \[
            \textrm{B} =\{a_{i_1},a_{i_2}, \dots, a_{i_k}\}
        \]
        \textbf{NB 1} $\dim$ C(A) $=|$B$|=k=$rk A\\
        \textbf{Nb 2} $\dim$C(A)$=$rk A$=$ rk U = $\dim$C(U)\\
        Ma C(A)$\neq$C(U) in generale, per cui \\
        $\vec{u}_{i_1}, \vec{u}_{i_2}, \dots, \vec{u}_{i_k}$ è una base di C(U) 
        {\color{red}Ma può essere una base di C(A)}\\
        \textbf{Esempio} A$= 
        \begin{bmatrix}
            1 & 2\\
            1 & 2
        ...
        \end{bmatrix}
        =$U\\\\
        Una base di C(A) è $\Bigg\{
            \begin{bmatrix} 
                1\\1
            \end{bmatrix} 
            \Bigg\}$\\
        Una base di C(U) è $\Bigg\{
            \begin{bmatrix} 
                1\\0
            \end{bmatrix} 
            \Bigg\}$\\\\
    ...
    \item A$=
        ...
        $\vec{r}_i^T\in \mathbb{C}_n$  \\
        U ha le prime $k$ righe non nulle (e le altre $m-k$ nulle)\\
        Si ha che R(A) $=$R(U) e che una base $D$ di R(A)$=$R(U) è 
        \[
            \textrm{D} = \{\overline{\vec{s}}_1,\overline{\vec{s}}_2,\dots,\overline{\vec{s}}_k\}\\
        \]
...
$-$rk A$=n-k$\\
    ...
    \paragraph{Proprietà del rango}\hspace{1cm}\\
...
Sia $\mathcal{B}=\{\vec{v}_1;\vec{v}_2;\dots;\vec{v}_n\}$ una \textbf{base ordinata} di $V$\\
e sia $\vec{v}\in V$\\
